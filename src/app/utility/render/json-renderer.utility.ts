...
export class JsonIterator implements IterableIterator<JsonRow> {
  ...
  constructor(json: any) {
    this.initValue = json;
    this.objStack.push({ nested: this.initValue, iterator: 0, depth: 0, path: '/' });
  }
  ...
  next(): IteratorResult<JsonRow, any> {
    ...
    if (Array.isArray(node.nested)) {
      row = this.processArray(node.nested, node.iterator, node.depth, node.path);
    } else if (typeof node.nested === 'object') {
      row = this.processObject(node.nested, node.iterator, node.depth, node.path);
    } else {
  ...
  processArray(arr: Array<any>, iterator: number, depth: number, path: string) {
    ...
    const row: JsonRow = { key: iterator.toString(), depth, path: path + iterator };
  ...
  processObject(obj: any, iterator: number, depth: number, path: string) {
    ...
    const row: JsonRow = { key: properties[iterator], depth, path: path + properties[iterator] };
